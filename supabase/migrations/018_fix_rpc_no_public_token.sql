-- =====================================================================
-- Migration 018 — Corrige RPC removendo referencia a public_token
-- A coluna public_token (migration 002) nunca foi aplicada em alguns
-- ambientes; a coluna correta para link publico e public_code (003).
-- Tambem suporta lookup por short_code e id (UUID).
-- =====================================================================
...
begin
  ...
  where public_code = p_code
     or short_code = p_code
     or (length(p_code) = 36 and p_code ~ '^[0-9a-f-]+$' and id::text = p_code)
  limit 1;
  if not found then
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
    ...
    'product', jsonb_build_object('nome', p.nome, 'categoria', p.categoria, 'icon', p.icon)
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
    ...
    'trabalhador', jsonb_build_object(
      'nome_completo', tp.nome_completo, 'discord_handle', tp.discord_handle, 'avatar', tp.avatar
    ),
    'items', (select coalesce(jsonb_agg(jsonb_build_object(
      'quantidade', ci.quantidade, 'product', jsonb_build_object('nome', cp.nome, 'icon', cp.icon)
...
-- Tambem corrige PedidoDetalhe que ainda lia public_token (na URL p/ Cliente)
-- ja foi corrigido em src/pages/PedidoDetalhe.jsx (fallback removido)
